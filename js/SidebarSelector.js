...
window.DarkHorse.SidebarSelector = (function () {
  ...
  function _render() {
    ...
    sortBar.append('button').attr('class', 'horse-sort-btn').attr('id', 'ss-sort-winrate-desc').text('Win% ↓')
      .on('click', () => { _sortMode = 'winRateDesc'; _updateSortButtons(); _renderList(); });
    sortBar.append('button').attr('class', 'horse-sort-btn').attr('id', 'ss-sort-winrate-asc').text('Win% ↑')
      .on('click', () => { _sortMode = 'winRateAsc'; _updateSortButtons(); _renderList(); });
  ...
  function _renderList() {
    ...
    const winRateByHorse = new Map();

    horses.forEach(h => {
      const records = State().getHorseData(h.HorseID) || [];
      const total = records.length;
      const wins = records.filter(r => +r._place === 1).length;
      winRateByHorse.set(h.HorseID, total ? wins / total : 0);
    });
    ...
    const filtered = filter
      ...
      : horses.slice();
    filtered.sort((a, b) => {
      if (_sortMode === 'rtg') return (+a.Rtg || 0) - (+b.Rtg || 0);
      if (_sortMode === 'winRateDesc') {
        const wrDiff = (winRateByHorse.get(b.HorseID) || 0) - (winRateByHorse.get(a.HorseID) || 0);
        return wrDiff || a.Name.localeCompare(b.Name);
      }
      if (_sortMode === 'winRateAsc') {
        const wrDiff = (winRateByHorse.get(a.HorseID) || 0) - (winRateByHorse.get(b.HorseID) || 0);
        return wrDiff || a.Name.localeCompare(b.Name);
      }
  ...
  function _updateSortButtons() {
    ...
    container.select('#ss-sort-winrate-desc').classed('active', _sortMode === 'winRateDesc');
    container.select('#ss-sort-winrate-asc').classed('active', _sortMode === 'winRateAsc');
  }
