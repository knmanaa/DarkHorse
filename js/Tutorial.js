...
window.DarkHorse.Tutorial = (function () {
  ...
  const STEPS = [
    ...
    },
    {
      ...
      body      : 'The primary view is a diverging bar chart: bars right of zero are value zones (market underestimates), bars left are trap zones (market over-backs). Use the "View as scatter" toggle when you want the detailed calibration curve.',
      placement : 'left',
    },
    {
      tab       : 'page-betting',
      target    : '#blind-spot-matrix',
      title     : 'Public Blind Spot Matrix',
      body      : 'Use filters (min races, class, distance) to focus on statistically meaningful horses. Bubble color shows ROI, size shows sample size, and hovering reveals detailed stats before clicking a horse to inspect further.',
      placement : 'right',
    },
    {
      tab       : null,
  ...
  let _welcome, _startTourBtn, _skipTourBtn;

  // ---- Public API ---------------------------------------------------------
  function init() {
    _buildDOM();
    const openBtn = document.getElementById('btn-tutorial');
    if (openBtn) openBtn.addEventListener('click', showWelcome);
    // Always show onboarding prompt when the site loads.
  ...
  function _buildDOM() {
    ...
    // Welcome card shown before the tour begins.
    _welcome = document.createElement('div');
    _welcome.className = 'tut-welcome';
    _welcome.setAttribute('role', 'dialog');
    _welcome.setAttribute('aria-modal', 'true');
    _welcome.setAttribute('aria-label', 'Welcome to DarkHorse');

    const welcomeTitle = document.createElement('h2');
    welcomeTitle.className = 'tut-welcome-title';
    welcomeTitle.textContent = 'Welcome to Dark Horse Race Analytic Dashboard';

    const welcomeDesc = document.createElement('p');
    welcomeDesc.className = 'tut-welcome-desc';
    welcomeDesc.textContent = 'Dark Horse is an interactive, multi-view dashboard for exploring Hong Kong horse racing data from the 2024–2025 HKJC season.';

    const welcomePrompt = document.createElement('p');
    ...
    _skipTourBtn = document.createElement('button');
    _skipTourBtn.className = 'tut-btn tut-btn-secondary';
    _skipTourBtn.textContent = 'Skip';
    _skipTourBtn.addEventListener('click', hideWelcome);

    _startTourBtn = document.createElement('button');
    _startTourBtn.className = 'tut-btn tut-btn-primary';
    _startTourBtn.textContent = 'Start Tutorial';
    _startTourBtn.addEventListener('click', start);

    welcomeActions.appendChild(_skipTourBtn);
    welcomeActions.appendChild(_startTourBtn);
    ...
    _overlay.appendChild(_welcome);
    document.body.appendChild(_overlay);

    // Keyboard navigation
    document.addEventListener('keydown', e => {
      if (!_active && e.key === 'Escape' && _overlay.classList.contains('welcome-active')) {
        hideWelcome();
        return;
      }
      if (!_active) return;
  ...
  function showWelcome() {
    ...
    _overlay.classList.add('welcome-active');
  }

  function hideWelcome() {
    _overlay.classList.remove('welcome-active');
    _overlay.style.display = 'none';
  }
